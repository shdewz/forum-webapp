...
def get_threads(board_id):
    sql = '''SELECT DISTINCT ON (threads.id)
             boards.id AS board_id, boards.name, threads.id, threads.title, messages.id AS message_id, messages.author_id, users.username AS author, u2.username AS last_reply, users.is_admin, messages.sent_at
             FROM boards
             ...
             LEFT JOIN users u2 ON u2.id = messages.author_id
             LEFT JOIN users u3 ON u3.id = :user_id
             LEFT JOIN permissions ON permissions.user_id = :user_id AND permissions.board_id = :board_id
             WHERE boards.id = :board_id AND (boards.is_public = true OR u3.is_admin = true OR permissions.user_id IS NOT null) ORDER BY threads.id, messages.sent_at DESC NULLS LAST'''
    result = db.session.execute(text(sql), {'board_id': board_id, 'user_id': users.user_id()})
