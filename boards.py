...
def get_boards():
    sql = '''SELECT DISTINCT ON (boards.id)
             boards.id, boards.name, boards.description, threads.id AS thread_id, threads.title AS thread_title, messages.id AS message_id, messages.author_id, users.username, users.usergroup, messages.sent_at
